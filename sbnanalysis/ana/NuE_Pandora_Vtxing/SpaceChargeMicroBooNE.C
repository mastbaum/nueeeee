...
{
  Configure("/uboone/app/users/jaz8600/NuMI_nue/nueeeee/sbnanalysis/ana/NuE_Pandora_Vtxing/SCEoffsets_MicroBooNE_E273.root");
}
...
//----------------------------------------------------------------------------
/// Check to see if point is inside FV boundaries 
bool SpaceChargeMicroBooNE::IsInsideFV(double xVal, double yVal, double zVal) const
{
  bool isInside = true;

  if((xVal < (0.0 + 20)) || (xVal > (256.35 - 20)) || (yVal < (-116.5 + 20)) || (yVal > (116.5 - 20)) || (zVal < (0.0 + 20)) || (zVal > (1036.8 - 20)))
  {
    isInside = false;
  }
